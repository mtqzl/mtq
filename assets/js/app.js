...
function renderUpcoming(data) {
  ...
  const heroImage = document.getElementById('upcoming-hero-image');
  const nextEventTitle = document.getElementById('upcoming-title');
  const nextEventMeta = document.getElementById('upcoming-meta');

  if (events.nextEvent) {
    if (heroImage) {
      const src = events.nextEvent.image ? encodeURI(events.nextEvent.image) : '';
      if (src) {
        heroImage.innerHTML = `
          <div class="hero-media">
            <img src="${src}" alt="${t(events.nextEvent.title)}" loading="eager" decoding="async">
          </div>
        `;
      } else {
        heroImage.innerHTML = '';
...
function renderPage() {
  ...
  if (page === 'contact') renderContact(state.data);
...
async function init() {
  try {
    ...
    const [band, events, discography, links, labels, contact] = await Promise.all([
    ...
    state.data = { band, events, discography, links, labels, contact };
    renderPage();
